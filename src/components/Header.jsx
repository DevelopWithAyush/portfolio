...
const Header = ({ setConnect }) => {
  ...
  const { user, logoutUser } = useContext(HandleContext);
  useEffect(() => {
    let lastScroll = 0;
  ...
  return (
    ...
    >
      ...
      {location.pathname === "/connect" ? (
        <button onClick={logoutUser} >       <ConnectButton buttonName={"Logout"} /></button>
      ) : user ? (
        <Link to={"/connect"}>
          <ConnectButton buttonName={"Connect"} />
        </Link>
      ) : (
        <button onClick={() => setConnect(true)}>
          <ConnectButton buttonName={"Login"} />
        </button>
      )}
    </header>
  );
  ...
  return (
    <>
      <nav className="hidden lg:flex flex-row items-center justify-center gap-[60px] capitalize ">
        <a
          className="text-[#FFF] text-[20px] font-medium"
        ...
        <a
          className="text-[#FFF] text-[20px] font-medium"
          href={location.pathname === "/" ? "#aboutme" : "/#aboutme"}
        >
          About Me
        </a>
        <a
          className="text-[#FFF] text-[20px] font-medium"
          href={location.pathname === "/" ? "#project" : "/#project"}
        >
          Project
        </a>
        <a
